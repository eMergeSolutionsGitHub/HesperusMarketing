...
//				columns, KEY_UPLOADED_STATUS+" = ?", new String[]{status}, null, null, null);
    ...
    public ArrayList<ReportList> getDataForReturn(String customer, String town,String fromDate,String toDate,int searchStatus){
        ArrayList<ReportList> returns = new ArrayList<>();
        Cursor cursor = null;

        if (searchStatus==0) {
            cursor = database.rawQuery("select cus.customer_name, ret.return_date, ret.return_invoice, ret.unit_price,inv.time_stamp from product_returns ret INNER JOIN customers cus on ret.pharmacy_id = cus.pharmacy_id INNER JOIN invoice inv on ret.return_invoice = inv.row_id where cus.customer_name = '" + customer + "' ORDER BY ret.return_invoice", null);

        } else if (searchStatus==1) {
            cursor = database.rawQuery("select cus.customer_name, ret.return_date, ret.return_invoice, ret.unit_price,inv.time_stamp from product_returns ret INNER JOIN customers cus on ret.pharmacy_id = cus.pharmacy_id  INNER JOIN invoice inv on ret.return_invoice = inv.row_id where cus.customer_name = '" + customer + "' AND ret.return_date BETWEEN '" + fromDate + "' AND '" + toDate + "' ORDER BY ret.return_invoice", null);

        } else if (searchStatus==3) {
            cursor = database.rawQuery("select cus.customer_name, ret.return_date, ret.return_invoice, ret.unit_price,inv.time_stamp from product_returns ret INNER JOIN customers cus on ret.pharmacy_id = cus.pharmacy_id  INNER JOIN invoice inv on ret.return_invoice = inv.row_id where cus.town = '" + town + "' ORDER BY ret.return_invoice", null);

        } else if (searchStatus==4) {
            cursor = database.rawQuery("select cus.customer_name, ret.return_date, ret.return_invoice, ret.unit_price,inv.time_stamp from product_returns ret INNER JOIN customers cus on ret.pharmacy_id = cus.pharmacy_id  INNER JOIN invoice inv on ret.return_invoice = inv.row_id where cus.town = '" + town + "' AND ret.return_date BETWEEN '" + fromDate + "' AND '" + toDate + "' ORDER BY ret.return_invoice", null);
        ...
        while (!cursor.isAfterLast()) {
            ...
            dealerSaleEntity.setDelName(cursor.getString(0));
            dealerSaleEntity.setReturnDate(cursor.getString(1));
            dealerSaleEntity.setInvoiceNo(cursor.getString(2));
            dealerSaleEntity.setValue(cursor.getString(3));
            dealerSaleEntity.setInvoiceDate(cursor.getString(4));

            returns.add(dealerSaleEntity);
            cursor.moveToNext();
        }
