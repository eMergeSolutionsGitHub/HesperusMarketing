...
public class InvoicedProducts {
    ...
    String[] columns = {KEY_ROW_ID, KEY_INVOICE_ID, KEY_PRODUCT_CODE, KEY_BATCH_NO, KEY_REQUEST_QTY, KEY_FREE, KEY_DISCOUNT, KEY_NORMAL, KEY_DATE, KEY_PRICE};
    ...
    public long insertInvoicedProducts(String invoiceId, String productCode, String batchNo, String requestQty, String free, String discount, String normal, String date, String price) throws SQLException {
    ...
    public List<String[]> getInvoicedProductsByInvoiceId(String invoiceId) {
        ...
        Cursor  cursor = database.rawQuery("SELECT * FROM invoiced_product ip inner join products p on ip.product_code = p.code WHERE invoice_id ='" + invoiceId + "' ORDER BY p.principle ASC ", null);


        // final String query = "SELECT * FROM " + TABLE_NAME + " WHERE " + KEY_INVOICE_ID + "='" + invoiceId + "' GROUP BY " + KEY_PRODUCT_CODE  ;

        //  Cursor cursor = database.rawQuery(query, null);

    //    Cursor cursor = database.query(TABLE_NAME, columns, KEY_INVOICE_ID + " = '" + invoiceId + "'", null, null,  KEY_DATE + " DESC", null);
...
//himanshu
public List<String[]> getInvoicedProductsByInvoiceIdFoBrand(String invoiceId,int stat) {
    List<String[]> invoicedProducts = new ArrayList<String[]>();
    Cursor  cursor;
    if(stat ==0){
