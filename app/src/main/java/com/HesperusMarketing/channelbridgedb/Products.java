...
public class Products {
    ...
    private static final String KEY_DES_PRINT = "descriptionPrint";
    private static final String KEY_PRINCIPLE_ORDER = "principleSortOrder";
    ...
    String[] columns = new String[]{KEY_ROW_ID, KEY_ID, KEY_CODE, KEY_BRAND,
            ...
            KEY_INACTIVE, KEY_VAT, KEY_TT, KEY_TIME_STAMP, KEY_PROD_ROW_ID, KEY_PRIORITY, KEY_SORTORDER, KEY_DES_PRINT, KEY_PRINCIPLE_ORDER};
    ...
    private static final String PRODUCTS_CREATE = "CREATE TABLE " + TABLE_NAME
            ...
            + KEY_FORCE + " TEXT NOT NULL ,"
            + KEY_INACTIVE + " TEXT ,"
            + KEY_VAT + " TEXT ,"
            + KEY_TT + " TEXT ,"
            + KEY_TIME_STAMP + " TEXT NOT NULL , "
            + KEY_PROD_ROW_ID + " TEXT , "
            + KEY_PRIORITY + " TEXT , "
            + KEY_SORTORDER + " INTEGER , "
            + KEY_DES_PRINT + " TEXT , "
            + KEY_PRINCIPLE_ORDER + " INTEGER " + " );";
    ...
    public static void onCreate(SQLiteDatabase database) {
        ...
        database.execSQL("CREATE INDEX productsRowid ON products(row_id);");
        database.execSQL("CREATE INDEX productsId ON products(id);");
        database.execSQL("CREATE INDEX productsCode ON products(code);");
        database.execSQL("CREATE INDEX productsBrand ON products(brand);");
        database.execSQL("CREATE INDEX productsUnitname ON products(unit_name);");
        database.execSQL("CREATE INDEX productsUnitsize ON products(unit_size);");
        database.execSQL("CREATE INDEX productsGenericname ON products(generic_name);");
        database.execSQL("CREATE INDEX productsCategory ON products(category);");
        database.execSQL("CREATE INDEX productsProdes ON products(pro_des);");
        database.execSQL("CREATE INDEX productsintroduceddate ON products(introduced_date);");
        database.execSQL("CREATE INDEX productcountoforigin ON products(count_of_origin);");
        database.execSQL("CREATE INDEX productunitprinciple ON products(principle);");
        database.execSQL("CREATE INDEX productspurchaseprice ON products(purchase_price);");
        database.execSQL("CREATE INDEX productssellingprice ON products(selling_price);");
        database.execSQL("CREATE INDEX productretailprice ON products(retail_price);");
        database.execSQL("CREATE INDEX productsforce ON products(force);");
        database.execSQL("CREATE INDEX productinactive ON products(inactive);");
        database.execSQL("CREATE INDEX productunitvat ON products(vat);");
        database.execSQL("CREATE INDEX productsgenerictt ON products(tt);");
        database.execSQL("CREATE INDEX producttimestamp ON products(time_stamp);");
        database.execSQL("CREATE INDEX productprodrowid ON products(prod_row_id);");
        database.execSQL("CREATE INDEX productpriority ON products(priority);");
        database.execSQL("CREATE INDEX productsortOrder ON products(sortOrder);");
        database.execSQL("CREATE INDEX productDesPrint ON products(descriptionPrint);");
        database.execSQL("CREATE INDEX productprincipleOder ON products(principleSortOrder);");
    ...
    public long insertProduct(String id, String brand, String code,
                              ...
                              String prodRowId, String priority, String sortOrder, String descriptionPrint, String principleOrder) throws SQLException {
        ...
        try {
            ...
            cv.put(KEY_SORTORDER, Integer.parseInt(sortOrder));
            cv.put(KEY_DES_PRINT, descriptionPrint);
            cv.put(KEY_PRINCIPLE_ORDER, Integer.parseInt(principleOrder));
        } catch (NumberFormatException n) {
            cv.put(KEY_SORTORDER, 0);
            cv.put(KEY_PRINCIPLE_ORDER, 0);
        }
    ...
    public List<String[]> getAllProducts() {
        ...
        Cursor cursor = database.query(TABLE_NAME, columns, null, null, null, null, KEY_FORCE + " ASC");
    ...
    public String[] getProductDetailsById(String id) {
        ...
        String[] productDetails = new String[25];

        Cursor cursor = database.rawQuery("SELECT * FROM products WHERE code = '" + id + "' ORDER BY principleSortOrder ASC ,pro_des ASC ", null);

        //  Cursor cursor = database.query(TABLE_NAME, columns, KEY_CODE + " = ?", new String[]{id}, null, null, KEY_PRINCIPLE + " ASC");
        //  Cursor cursor = database.query(TABLE_NAME, columns, KEY_CODE + " = ?", new String[]{id}, null, null, null);
        ...
        while (!cursor.isAfterLast()) {
            ...
            productDetails[19] = cursor.getString(19);
            productDetails[20] = cursor.getString(20);
            productDetails[21] = cursor.getString(21);
            productDetails[22] = cursor.getString(22);
            productDetails[23] = cursor.getString(23);
...
//		Cursor cursor = database.rawQuery("SELECT * from "+TABLE_NAME+" where "+KEY_CODE+" ='"+code+"' ", null);
    ...
    public long updateProduct(String id, String brand, String code,
                              ...
                              String sellingPrice, String retailPrice, String force,
                              String inactive, String vat, String tt, String timeStamp,
                              String prodRowId, String priority, String sortOrder) throws SQLException {
        ContentValues cv = new ContentValues();
        try {
            cv.put(KEY_ID, id);
//		cv.put(KEY_CODE, code);
    ...
    public ArrayList<String> getPrincipleList() {
        ...
        Cursor cursor = database.rawQuery("select distinct principle from products order by principleSortOrder ASC ", null);
   ...
   /* public ArrayList<Product> getProductsByPricipleAndCategory(String principle, String category) {
        ...
        // Cursor cursor = database.rawQuery("SELECT * FROM products p INNER JOIN productRepStore pr ON p.code = pr.product_code AND p.principle ='" + principle + "' AND p.category ='" + category + "' order by p.sortOrder ASC ", null);

        final String MY_QUERY = "SELECT * FROM products p INNER JOIN productRepStore pr ON p.code = pr.product_code WHERE p.principle=? AND p.category = ? order by pr.product_id ASC";

        Cursor cursor = database.rawQuery(MY_QUERY, new String[]{principle, category});
        // Cursor cursor = database.rawQuery("select * from products inner join productRepStore  on products.code = productRepStore.product_code  And principle = ? and category = ? order by products.sortOrder ASC", new String[]{principle, category});

        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            Product productData = new Product();

            productData.setRowId(Integer.parseInt(cursor.getString(25)));// rowId of repstore21
            productData.setId(cursor.getString(26));// product id from rep stores22
            ...
            productData.setPurchasePrice(Double.parseDouble(cursor.getString(31)));// purchase Price27
            productData.setSellingPrice(Double.parseDouble(cursor.getString(32)));// selling Price----28
            ...
            productData.setPriority(cursor.getString(23));//26
            productData.setSort(cursor.getInt(24));//26
...
//            productData.setUnitSize(); cursor.getString(20);// tt
    ...
    public ArrayList<Product> getProductsByPricipleAndCategory(String principle, String category) {
        ...
        final String MY_QUERY = "SELECT p.code,p.pro_des,pr.batch_number,p.selling_price FROM products p INNER JOIN productRepStore pr ON p.code = pr.product_code WHERE p.principle=? AND p.category = ? order by pr.product_id ASC";
        Cursor cursor = database.rawQuery(MY_QUERY, new String[]{principle, category});
        ...
        while (!cursor.isAfterLast()) {
            ...
            productData.setBatchNumber(cursor.getString(2));//24
            productData.setSellingPrice(Double.parseDouble(cursor.getString(3)));// selling Price----28
    ...
    //Himanshu
    public ArrayList<Product> getProductsByPriciple(String principle) {
        openReadableDatabase();
        ArrayList<Product> products = new ArrayList<>();

        // Cursor cursor = database.rawQuery("SELECT * FROM products p INNER JOIN productRepStore pr ON p.code = pr.product_code AND p.principle ='" + principle + "' AND p.category ='" + category + "' order by p.sortOrder ASC ", null);

        final String MY_QUERY = "SELECT * FROM products p INNER JOIN productRepStore pr ON p.code = pr.product_code WHERE p.principle=? order by pr.product_id ASC";

        Cursor cursor = database.rawQuery(MY_QUERY, new String[]{principle});
        // Cursor cursor = database.rawQuery("select * from products inner join productRepStore  on products.code = productRepStore.product_code  And principle = ? and category = ? order by products.sortOrder ASC", new String[]{principle, category});
...
//            productData.setUnitSize(); cursor.getString(20);// tt
    ...
    public String getRetailsPrice(String code) {
        openReadableDatabase();
        Cursor cursor = database.rawQuery("SELECT retail_price from products where code = ?", new String[]{code});
        cursor.moveToFirst();
        return cursor.getString(0);
    }
